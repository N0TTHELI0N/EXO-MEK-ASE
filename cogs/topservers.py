...
class TopServers(commands.Cog):
    ...
    @app_commands.command(name="top-servers", description="View top ARK servers by player count")
    ...
    async def top_servers(self, interaction: discord.Interaction, game: app_commands.Choice[str] = None, count: int = 10):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                params = {
                    "filter[game]": game_val,
                    "sort": "-players",
                    "page[size]": count,
                }
                async with session.get(BATTLEMETRICS_API, params=params) as resp:
                    if resp.status != 200:
        ...
        for i, s in enumerate(servers):
            ...
            ip_addr = attrs.get("ip", "")
            port = attrs.get("port", "")
            players = attrs.get("players", 0)
            max_players = attrs.get("maxPlayers", 0)
            status = attrs.get("status", "unknown")
            emoji = "🟢" if status == "online" else "🔴"
            lines.append(f"**#{rank}** {emoji} {name}\n`{ip_addr}:{port}` — 👥 {players}/{max_players}")
